...
//  MyNewProject
//
//  Created by Aitzhan Ramazan on 21.04.2025.
//
...
class MainPageViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, MovieProtocol {
    ...
    func downloadGenres() {
        ...
        AF.request(Urls.GET_GENRES_URL, method: .get, headers: headers).responseData {
            response in
            ...
            if response.response?.statusCode == 200 {
                ...
                if let array = json.array {
                    ...
                    for item in array {
                        ...
                        movie.categoryAge.append(ageCategory)
                    }
                    if self.mainMovies.count > 8{
                        if self.mainMovies[1].cellType == .ageCategory{
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if mainMovies[indexPath.row].cellType == .ageCategory || mainMovies[indexPath.row].cellType == .genre{
            return 184.0
        }
        //mainmovie
        return 288.0
